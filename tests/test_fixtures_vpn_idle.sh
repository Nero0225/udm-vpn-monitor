...
@test "vpn_idle fixture: sets up idle tunnel with static bytes and ping success" {
	...
	local last_bytes
	last_bytes=$(get_peer_state "TEST" "192.168.1.1" "last_bytes" "0" 2>/dev/null || echo "0")
	...
	run "${TEST_DIR}/ping" -c 1 192.168.1.1 2>&1 || true
	assert_success
...
@test "vpn_idle fixture: custom bytes and internal IP" {
	...
	setup_vpn_idle_fixture "192.168.1.1" 5000 "10.0.0.2"
...
@test "vpn_idle fixture: custom SPI value" {
	...
	setup_vpn_idle_fixture "192.168.1.1" 1000 "10.0.0.1" "0xABCD1234"

	# Verify state file has custom SPI
	ensure_state_functions_loaded
	local spi
	spi=$(get_peer_state "TEST" "192.168.1.1" "spi" "" 2>/dev/null || echo "")
...
@test "vpn_idle fixture: accepts additional config variables" {
	...
	setup_vpn_idle_fixture "192.168.1.1" 1000 "10.0.0.1" "0x12345678" 'TIER1_THRESHOLD=2' 'TIER2_THRESHOLD=4'
